...
The public repository also tracks the compact
[`run manifest`](run_manifest.csv), [`replication KPI table`](replication_kpis.csv),
[`strict validation report`](validation.json), and
[`audit manifest`](audit_manifest.json). The full 61,218-row traveller ledger
is regenerated by the workflow rather than committed; its row count and
SHA-256 are frozen in the audit manifest.
